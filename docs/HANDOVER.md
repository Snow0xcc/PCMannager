...
本轮（面板双传输、版本注入、模块失败原因、热键改绑）在 Linux 下已验证：
四平台 `CGO_ENABLED=0` 构建、`go vet`、`gofmt`、emoji 检查、202 用例 `-race`
全绿。但**以下四项只能在真实 Windows 上确认**，当前尚未验证。

产物构建（与 CI 同源，勿直接用 `go build`，否则丢 `-H windowsgui`）：

```
PCM_VERSION=v0.1.0-rc2 bash scripts/build.sh windows amd64 dist/pcmannager-windows-amd64.exe
```
